...
test.describe
  .serial('/api-i/chat', () => {
    test('Ada cannot invoke a chat generation with empty request body', async ({
      ...
      const response = await adaContext.request.post('/api-i/chat', {
        data: JSON.stringify({}),
      });
    ...
    test("Babbage cannot append message to Ada's chat", async ({
      ...
      const response = await babbageContext.request.post('/api-i/chat', {
        data: {
          id: chatId,
    ...
    test('Ada cannot resume stream of chat that does not exist', async ({
      ...
      const response = await adaContext.request.get(
        `/api-i/chat/${generateUUID()}/stream`,
      );
    ...
    test('Ada can resume chat generation', async ({ adaContext }) => {
      ...
      const firstRequest = adaContext.request.post('/api-i/chat', {
        data: {
          id: chatId,
          message: {
            id: generateUUID(),
    ...
    test('Ada can resume chat generation that has ended during request', async ({
      ...
      const firstRequest = await adaContext.request.post('/api-i/chat', {
        data: {
          id: chatId,
          message: {
            id: generateUUID(),
    ...
    test('Ada cannot resume chat generation that has ended', async ({
      ...
      const firstResponse = await adaContext.request.post('/api-i/chat', {
        data: {
          id: chatId,
          message: {
            id: generateUUID(),
