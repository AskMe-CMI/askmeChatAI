...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadAuthState = async () => {
      try {
        ...
        const response = await fetch('/api-i/auth/verify', { method: 'GET' });
  ...
  const login = async (email: string, password: string) => {
    try {
      const response = await fetch('/api-i/auth/login', {
        method: 'POST',
  ...
  const logout = () => {
    ...
    fetch('/api-i/auth/logout', { method: 'POST' }).catch(console.error);
  };
