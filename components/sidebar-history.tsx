...
export function getChatHistoryPaginationKey(
  ...
  // Stop pagination if previous page indicates no more data
  if (previousPageData && previousPageData.hasMore === false) {
    return null;
  }

  // For first page, start with offset 0
  if (pageIndex === 0) return `/internal-api/history?limit=${PAGE_SIZE}&offset=0`;

  // For subsequent pages, use offset-based pagination
  const offset = pageIndex * PAGE_SIZE;
  return `/internal-api/history?limit=${PAGE_SIZE}&offset=${offset}`;
}
...
export function SidebarHistory({
  ...
  // Only show "reached end" message if there are multiple chats
  const totalChats = paginatedChatHistories
    ? paginatedChatHistories.flatMap((page) => page?.chats || []).length
    : 0;

  const shouldShowEndMessage = hasReachedEnd && totalChats > 5;

  const handleDelete = async () => {
    console.log('✅ User confirmed deletion for chat:', deleteId);
    console.log('🚀 Starting delete process...');

    const deletePromise = fetch(`/internal-api/chat?id=${deleteId}`, {
      method: 'DELETE',
    });
          ...
          <SidebarMenu className="group-data-[collapsible=icon]:group-data-[state=collapsed]:hidden">
            {paginatedChatHistories &&
              (() => {
                ...
                // Deduplicate chats by ID to prevent showing duplicates
                const uniqueChats = Array.from(
                  new Map(
                    chatsFromHistory.map((chat) => [chat.id, chat]),
                  ).values(),
                );
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {groupedChats.today.map((chat) => (
                          <ChatItem
                            ...
                            onDelete={(chatId) => {
                              console.log(
                                '🗑️ Delete button clicked for chat:',
                                chatId,
                              );
                              setDeleteId(chatId);
                              setShowDeleteDialog(true);
                              console.log(
                                '💬 Showing delete confirmation dialog: "Are you absolutely sure?"',
                              );
                            }}
